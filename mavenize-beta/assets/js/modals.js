$(document).ready(function() {
  ...
  var createRaveButtons = function() {
    var ratingGroup = $('<div/>', { "class": "btn-group", "id": "modal-rating", "data-toggle": "buttons-radio" });
    for (var i = 1; i < 5; i++) {
      ratingGroup.append(
        $('<button/>', { "class": "btn", "type": "button", "name": "rating", "value": i }).append(
          $('<img/>', {"src": STATIC_URL + "img/" + i + "s.png" }))
        );
    }
    var submitButton = $('<button/>', { "class": "btn btn-large btn-primary disabled", "id": "modal-submit", "type": "submit", "name": "rating", "value": "0", "disabled": "true"}).text("Rave");

    return $(ratingGroup).after(submitButton).after($('<div/>', { "style": "clear: both;" }));
  }
